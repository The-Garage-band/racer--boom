...
export function startRacing(
  scope: GamePage,
  startGameLives: number,
  startGameScores: number,
  startAddHealth: string
) {
  if (started) return
  started = true
  ...
  let gameTimer: any = null,
    ...
    speedUpdater: any = null

  let gameScores = startGameScores,
    gameLives = startGameLives //Очки и жизни
    ...
    if (objRandomByNum > coinProb) {
      ...
      if (isPush) {
        ...
        if (objRandomByNum > coinProb && objRandomByNum < carProb) {
          gameObjects.push(
            new GameObject(coinImage, objX, objY, false, 10, false, 0)
          )
        }
        ...
        if (objRandomByNum > carProb && objRandomByNum < truckProb) {
          gameObjects.push(
            new GameObject(grayCarImage, objX, objY, false, 0, false, 0)
          )
        }
        ...
        if (objRandomByNum > truckProb && objRandomByNum < policeProb) {
          gameObjects.push(
            new GameObject(truckCarImage, objX, objY, false, 0, false, 0)
          )
        }
        ...
        if (objRandomByNum > policeProb && objRandomByNum < liveProb) {
          gameObjects.push(
            new GameObject(policeCarImage, objX, objY, false, 0, true, 0)
          )
        }
        ...
        if (objRandomByNum > liveProb) {
          gameObjects.push(
            new GameObject(liveImage, objX, objY, false, 0, false, 1)
          )
        }
      }
    }
