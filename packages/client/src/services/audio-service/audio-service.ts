...
export class AudioService {
  ...
  private constructor () {
    ...
    if (currentSound && SOUNDS.includes(currentSound)) {
      const filteredSound = Random.shuffleArray(SOUNDS.filter(sound => sound !== currentSound));
  ...
  public next = () => {
    ...
    this._currentSoundIndex = (this._currentSoundIndex + 1) % this._soundQueue.length;
    if (this._currentSoundIndex === 0) {
      ...
      this._soundQueue.splice(Random.randomInt(1, this._soundQueue.length), 0, lastSound);
    }
